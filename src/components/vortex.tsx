...
export const Vortex = (props: VortexProps) => {
  ...
  const backgroundColorRef = useRef(props.backgroundColor || "#000000");
  const blendModeRef = useRef<GlobalCompositeOperation>(
    props.blendMode || "lighter"
  );
  ...
  const renderGlow = (
    ...
    const blendMode = blendModeRef.current;
    const applyGlowPass = (blur: number, brightness: number) => {
      ctx.save();
  ...
  useEffect(() => {
    backgroundColorRef.current = props.backgroundColor || "#000000";
  }, [props.backgroundColor]);

  useEffect(() => {
    blendModeRef.current = props.blendMode || "lighter";
  }, [props.blendMode]);
